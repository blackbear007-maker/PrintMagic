...
一般新手或繪師將手機照片或 AI 生成圖拖入系統，自動完成尺寸比例適配、300 DPI 補齊、依版型出血（A4/A3 3mm、明信片與貼紙 2mm、名片 1.5mm、證件照與社群圖 0）、暗部防死黑、總墨量 TAC 壓制與純黑 K100 向量轉曲，一鍵下載含出血與裁切線的印刷用 PDF。PDF 會依選定的色彩描述檔（預設 Japan Color 2001 Coated）分色成 CMYK；分色服務無法使用時（本機模式、後端離線）改輸出 RGB，由印刷廠轉檔。下載時自動複製到剪貼簿的送印小抄會照實寫出是哪一種。
...
├───────────────────────────────┴───────────────────────────────┴───────────────────────────────┤
│ 10. ✅ 印刷用 PDF (CMYK)：內嵌向量角線、十字規矩線、出血框（分色服務離線時為 RGB）                │
└───────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
- **進階模式**：同一個畫面、同一張圖，評分卡上直接列出處理開關（放大、銳化、漸層防斷階、手機翻拍均光、總墨量壓制、暗部提亮、自動出血、貼紙自動去背），切換後立即重新處理。卡片上的解析度／出血／墨量／色彩說明依實際開關狀態與目前色彩描述檔的 TAC 上限產生。
