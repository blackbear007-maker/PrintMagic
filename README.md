...
一般新手或繪師將手機照片或 AI 生成圖拖入系統，自動完成尺寸比例適配、300 DPI 補齊、依版型出血（A4/A3 3mm、明信片與貼紙 2mm、名片 1.5mm、證件照與社群圖 0）、暗部防死黑、總墨量 TAC 壓制與純黑 K100 向量轉曲，一鍵下載含出血與裁切線的印刷用 PDF。⚠️ PDF 內容是 RGB，CMYK 轉換仍需由印刷廠處理；下載時自動複製到剪貼簿的送印小抄會照實告訴印刷廠這一點。
...
- **簡易模式（預設）**：丟圖後自動處理，結果卡只顯示「修正前 → 修正後」評分和一顆「下載印刷檔 (PDF)」，不列出做了哪些處理。畫布工具列、處理說明、單張畫廊都不顯示；手機上評分在圖片上方、下載按鈕固定在底部。小象會提示「想自己決定要套用哪些處理，切到【進階】」。下載後不再跳出彈窗，改由小象提醒檔案是 RGB。
