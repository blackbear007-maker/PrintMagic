...
- **測試**：刪除「保證會過」的 stress／benchmark（原本占整套測試 168 秒中的 139 秒），會在正式流程執行的濾鏡改成真的檢查輸出。整套 287 個測試約 4 秒。

仍待處理：手動工具（去網紋、去區塊、髮絲去背、證件照裁切）的結果在任何一次重新處理後會被丟掉；沒有任何測試呼叫產生前後分數的 `runOptimizationPipeline`；音效沒有靜音開關。
