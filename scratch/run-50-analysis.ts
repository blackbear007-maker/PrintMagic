...
const results = testCases.map((tc) => {
  ...
  const optimizedStats: ImagePixelStats = {
    ...
    edgeScore: Math.max(0.045, tc.edgeScore * 1.6),
    transparentRatio: 0,
    width: optimizedWidth,
    height: optimizedHeight
  };
  const optimizedInk: InkAnalysis = {
    maxTotalInk: Math.min(300, tc.maxTac),
  ...
  return {
    ...
    name: tc.name,
    dimensions: `${tc.width}×${tc.height}`,
    initialDpi: dpiAnalysis.currentDpi,
    initialTac: tc.maxTac,
    beforeScore: initialScore.score,
    afterScore: optimizedScore.score,
    delta: optimizedScore.score - initialScore.score
  };
});
