...
> ⚠️ **審視與修正（2026-09-24）**：
>
> - **出血**：`BleedExpander` 右、下兩邊的鏡像條起點算錯，畫回原圖範圍後被中心圖蓋掉，右／下出血一直是全透明（印出來是白邊）；原本的測試整個 canvas 都是 mock，所以一直通過。已修正，改用會真的寫像素的軟體 canvas 測試。
> - **不實文案**：下載 PDF 時自動複製的送印小抄寫「色彩：CMYK · 零退件認證」，實際是 RGB PDF——印刷廠照做就不會轉檔。已改為照實寫 RGB 並請印刷廠轉檔；「規格小抄」彈窗同步修正。小象的貼紙／燙金台詞宣稱 PDF 內含白墨層、刀模線與燙金版，實際都沒有，已改寫。評分結論不再同時出現「完美就緒」與「N 項待改善」。
> - **假開關**：「ICC 色彩映射軟打樣」（`enableColorProofing`）管線從未讀取，已從介面移除；補上真正會生效的「漸層防斷階」「手機翻拍均光」開關。
...
> ⚠️ **全面審查與修正（2026-09-18～19）**：以 18 個平行審查代理人分區檢查全部原始碼、伺服器與 Docker 設定，每個疑似問題再交給獨立代理人嘗試推翻，最後確認 **80 個真實問題**（高 17、中 33、低 30），另有 5 個被驗證推翻。修正後測試由 321 項增為 336 項，全數通過。重點如下：
>
> - **輸出錯誤**：雙面 PDF 用錯正面圖且背面不會清除、一般批次上傳被自動配成正反面；再次套用文字疊加會疊出兩份文字；自動選擇 ICC 描述檔只改了下拉選單，壓墨上限仍用舊描述檔；連續擦除物件會丟掉前一次結果；「補足出血」固定加 3mm 且可重複疊加；裁切線畫進出血區壓在圖上；TIFF 的 IFD 與 RATIONAL 落在奇數位移；ZIP 宣稱含 PDF 實際沒有。
> - **濾鏡**：USM 遞迴高斯零初始值造成邊框變亮、L* 單位直接加到 sRGB 值上；去模糊核心總和 1.04 且留下 2px 透明黑邊；Lanczos 未預乘 alpha 導致透明邊緣滲色；線稿放大在 scale=1 時仍重取樣並強制 alpha=255。
> - **競態與資源**：處理流程加上執行編號，較舊的處理結果不再覆蓋新圖／新版型；預覽快取在就地編輯後會失效；文字檢查結果不再帶到下一張圖；下載連結延後釋放避免 Safari 下載失敗；逾時計時器一律在 finally 清除。
> - **安全**：提示訊息以 innerHTML 寫入小象對話框（檔名可注入 HTML）、OCR 文字的屬性注入；伺服器 CORS 改為白名單、限制請求大小；zero-dce 在完整解碼前先檢查像素數（防解壓縮炸彈）。
> - **服務與部署**：vtracer 包裝層傳了 CLI 不接受的參數導致每次向量化都失敗、表單參數與 Node 代理傳法不一致；正式 Dockerfile 漏了 `src/` 導致 `/api/prepress/*` 找不到模組；PWA 設定檔被 Vite 打包進 `/assets/` 導致圖示與啟動網址指錯，改為直接使用 `public/manifest.json`；啟動時會把自己剛註冊的 Service Worker 解除，離線功能從未生效，已修正並刪除過時的根目錄 `sw.js`。
> - **行為變更（刻意的取捨）**：(1) **本機模式真正不上傳圖片**，判斷集中在 `NetworkGuard.isRemoteAllowed()`；代價是本機模式下擦除、去背、低光提亮改用本機演算法，人臉偵測與精準 ICC 軟打樣無法使用。(2) **手機翻拍光照均勻化（`HandShadowBalancer`）預設關閉**，原本對每張圖都執行、會把刻意的暗部提亮；改在進階模式提供開關，且暗圖一律不處理。(3) 放大倍率顯示實際輸出寬度／原圖寬度，不再宣稱固定 8x／4x；AI 放大不足目標時以 Lanczos 補足。(4) 壓力測試的單輪耗時上限只在設定 `PERF_GUARD` 時檢查，平時改驗中位數。
> - **誠實性**：移除「100% 離線」「商業作品不聯網」「8x 金字塔超解析度」「Potrace 平滑曲線」、固定「3mm 出血」等與實際不符的介面與文件說法；PrintPass 報告不再無條件打勾；壓力測試腳本不再印出寫死的「100% 通過」。
> - **已知但未處理**：`CmykEngine` 的分色輸出總墨量最高約 200%，因此 300% 的 TAC 上限在實務上幾乎不會觸發，墨量熱力圖已改以可達範圍計算；真正支援高墨量深黑需要改寫分色模型。Python 服務（vtracer、zero-dce）與 Dockerfile 的修改只做過語法檢查，尚未實際建置映像檔驗證。
>
> **介面與品牌改版（2026-09-18～19）**：頁面結構改由 `src/ui/app-shell.ts` 以 TypeScript 產生；新 LOGO（大象頭一筆畫成字母 P，向量主檔 `public/brand/logo-mark.svg`）；全站「紙與油墨」視覺主題（`src/styles/theme-paper.css`）；首頁簡化為單一上傳按鈕。詳見第 8 節。
...
- **`id-photo-cropper.ts` (2 吋護照大頭照頭頂自動校準)**：精確定位台灣與國際 2 吋大頭照 3.2~3.6cm 官方頭部比例。
...
- **`bleed-expander.ts` (依版型智慧出血背景生長)**：鏡像外推 + 接縫混合，依版型出血（A4/A3 3mm、明信片與貼紙 2mm、名片 1.5mm、證件照與社群圖 0），杜絕裁切白邊與文字切除。
- **`cmyk-engine.ts` / `ink-limiter.ts` (控墨防死黑、軟打樣)**：物理減法混色預測 + 總墨量 TAC 上限壓制（2026-08-28 修正 GCR 與 TAC 計算錯誤，見文件頂端附註）。⚠️ 分色輸出的總墨量最高約 200%，300% 上限實務上幾乎不會觸發，見文件頂端 2026-09-18 附註。
...
### 2.2 出血外推與修補
- **`bleed-expander.ts`**：鏡像外推 + 接縫余弦混合 + 四角交叉修補 + 接縫色溫微調，生成依版型出血量的印刷出血。（2026-09-24 修正右／下出血全透明的錯誤，見文件開頭附註。）
...
- **`hand-shadow-balancer.ts`**：24×24 網格光照插值，均化手機翻拍陰影。2026-09-18 起**預設關閉**，在進階模式勾選「☀️ 手機翻拍光照均勻化」才會執行；畫面中位亮度偏暗時一律不處理，避免把刻意的暗部提亮。
...
- **`ink-limiter.ts`**：總墨量 (TAC) 上限壓制（2026-08-28 修正雙重計算灰階份量導致的 TAC 虛報錯誤）。上限取自目前選擇的色彩描述檔；2026-09-18 修正自動選擇描述檔時實際上限沒有跟著切換的問題。
...
### 2.8 文字、條碼與防呆
- **`text-inspector.ts`**：文字區域偵測（2026-09-24 重寫為筆畫交叉數小格分類 + 文字行組裝，含重複圖案／QR／圓形輪廓排除；基準測試 `tests/text-region-detection.test.ts`；已知限制：直書中文、部分單字與短數字、低對比或純色相差異、透明背景文字）+ **真實本機 OCR**（2026-08-29 新增，見下方獨立段落）+ 錯字檢查。錯字檢查完全是本機的，透過 `free-spellcheck-client.ts` 的 ~18 條正規表示式規則比對——儘管兩個檔案的舊版註解都聲稱有打 LanguageTool 免費 API，實際上整條路徑裡沒有任何一次網路請求，純粹是本機規則比對，已於 2026-08-25 修正說明文字。過去這裡的 OCR 檢查一直是無法觸發的死路（`region.text` 恆為空字串），現在因為真的有 OCR，終於第一次真正跑起來。
...
├─────────────────────────────┼────────────────────────────────────────────────────────┤
│ 2. 📷 寫實人像 / 婚紗寫真   │ ➔ EdgeAwareUpscaler 放大 + SmoothingDenoiseFilter 去噪（HandShadowBalancer 需手動開啟） │
├─────────────────────────────┼────────────────────────────────────────────────────────┤
...
1. **預設簡易模式 (Simple Mode by Default，2026-09-24 改版)**：
   - 結果卡只顯示「修正前 → 修正後」評分與一顆「下載印刷檔 (PDF)」；不列出處理細節（原本的「10 大守護」徽章網格已移除）。畫布工具列、畫布分數標籤、背面提示、單張畫廊只在進階模式出現。手機上評分在圖片上方，下載按鈕為底部浮動列。
   - 切到進階模式時保留同一個畫面與圖片，評分卡直接列出處理開關，切換即重新處理；卡片說明依實際開關與目前 ICC 描述檔 TAC 上限產生。
2. **0-Click 全自動背景流水線**：
   - 丟圖即自動完成：比例適配、300DPI 補齊、依版型出血、暗部防死黑、TAC 上限控墨、K100 文字純黑。
   - 首頁（2026-09-19 簡化）：只保留「從相簿選擇相片」一個按鈕，也可直接拖放；已移除印前術語標籤、拍照掃描與貼上剪貼簿按鈕、音效開關。
...
## 8. 介面設計與品牌 (UI & Brand，2026-09-19)

### 8.1 LOGO
- **概念**：大象頭用一筆等粗線畫出字母 P（Print），帶象牙與象鼻；極簡、單色。
- **檔案**（`public/brand/`）：`logo-mark.svg`（專色深紫 `#3c1e8c`）、`logo-mark-black.svg`（單色黑，燙印／單色印刷）、`logo-mark-white.svg`（反白，深色底）、由 SVG 輸出的 `logo-mark-64/192/512.png` 與 `apple-touch-icon.png`。
- **印刷規則**：線寬 50 單位（約 LOGO 高度 9.6%）、所有空隙至少 40 單位、象牙尖端為 32 單位圓頭；印到約 1 公分高時最細處約 0.6mm。概念稿由 Vertex AI 生成，定稿以幾何線條重新構成向量，而非直接描圖。
- **使用位置**：頁首品牌、瀏覽器分頁圖示、iPhone 主畫面圖示、PWA 安裝圖示（`public/manifest.json`）。

### 8.2「紙與油墨」視覺主題
- **實作**：`src/styles/theme-paper.css`，在其他樣式之後載入，以覆寫設計基準值（CSS 變數）為主、再針對主要元件調整。
- **色彩**：紙 `#f6f2ea`、墨 `#221b2e`、專色深紫 `#3c1e8c`（主要動作）、朱紅 `#b8432b`（送印動作）。原本散落各處寫死的蘋果藍與另一種紫已全部換成專色深紫。
- **素材**：`public/textures/paper.webp`（可無縫平鋪的棉紙紋）與 `ink-wash.webp`（右上角深紫油墨暈染），由 Vertex AI（Gemini 生圖）生成後處理成 WebP；上傳區四角的裁切線以 CSS 繪製，任何解析度都清晰。
- **字體**：標題 Noto Serif TC（Google Fonts，會連線下載字型但不上傳圖片；失敗時退回系統明體），介面文字維持系統無襯線字。
- **手機版**：頁首工具列可橫向滑動，品牌名維持單行；中文標題以詞組為單位換行，不會把「印刷」拆開。

### 8.3 版面程式結構
- `index.html` 只保留 `<head>` 與模組載入；頁面結構由 `src/ui/app-shell.ts` 的 `APP_SHELL_HTML` 產生，在 `bootstrapApp()` 一開始由 `mountAppShell()` 掛上，之後才初始化其他元件。所有元素 ID 與 class 與改寫前相同。
- Service Worker 只保留 `public/sw.js` 一份，快取版本隨素材更新（目前 `printmagic-v3.4.0-offline`），並預先快取 LOGO 與紙紋素材。
